...
/// `merge_report`'s own return type, named so its signature stays
/// readable - a merged map plus every real conflict it resolved, or the
/// one real collision (see `IdentityCollision`) it refused to resolve.
type MergeReportResult<K, V> =
    Result<(LwwMap<K, V>, Vec<MergeConflict<K, V>>), IdentityCollision<K, V>>;
...
impl<K: Ord + Clone, V: Clone + PartialEq> LwwMap<K, V> {
    ...
    /// it commutative, associative and idempotent under this map's own
    /// single-writer invariant (a given (time, writer) stamp is only ever
    ...
    /// time, or a malicious peer - this fast, infallible path has no
    /// principled Stamp-based way to pick a winner and silently keeps
    /// `self`'s value, which is a real, order-dependent break in the
    /// commutativity claim above (`self.merge(other)` and
    /// `other.merge(self)` would then disagree). `merge()` stays this
    /// way deliberately - a caller that needs that anomaly actually
    /// detected and rejected, not silently papered over, uses
    /// `merge_report` instead, which is what this project's own real
    /// reconciliation path (`reconcile_with_prior`) always uses.
    ...
    /// that `self` never had) - and a real, explicit REJECTION (`Err`)
    /// rather than a merged result at all if it finds an identical stamp
    /// claiming two different values (see `IdentityCollision`'s own
    /// header comment). This is the safe, opt-in entry point for a
    /// caller that wants real visibility into what a reconciliation
    /// actually overwrote, e.g. an operator asking which node's write
    /// just got discarded and by whom after two swarm cells reconnect.
    /// This project's own real reconciliation path
    /// (`reconcile_with_prior`) always uses this, never plain `merge()`.
    pub fn merge_report(&self, other: &Self) -> MergeReportResult<K, V> {
        let mut result = self.clone();
            ...
            match result.entries.get(key) {
                Some(existing) if existing.stamp == other_entry.stamp => {
                    if existing.value != other_entry.value {
                        ...
                        // order - there is no principled winner).
                    ...
                    // Identical stamp AND identical value - a true
                    // idempotent duplicate (e.g. a retried gossip round
                    // delivering the same update twice). Not a conflict.
                }
                Some(existing) => {
...
/// either, is safe to keep by hand.
#[derive(Debug, Clone, PartialEq)]
pub struct IdentityCollision<K, V> {
    pub key: K,
...
impl<K: std::fmt::Debug, V: std::fmt::Debug> std::fmt::Display for IdentityCollision<K, V> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "impossible conflict on key {:?}: writer {} claims two different values ({:?} vs {:?}) at the identical logical time {:?} - refusing to guess a winner",
            self.key, self.writer, self.local_value, self.remote_value, self.time
        )
    }
}

impl<K: std::fmt::Debug, V: std::fmt::Debug> std::error::Error for IdentityCollision<K, V> {}
...
mod tests {
    ...
    fn merge_report_never_flags_a_re_merge_of_an_identical_stamp_as_a_conflict() {
        ...
        let mut a: LwwMap<String, i32> = LwwMap::new();
        a.set("x".to_string(), 1, LamportTime(1), 1);
        let (_, conflicts) = a.merge_report(&a.clone()).unwrap();
        assert!(conflicts.is_empty());
    }
    ...
    fn merge_report_matches_plain_merge_result() {
        ...
        let (via_report, conflicts) = a.merge_report(&b).unwrap();
    ...
    fn merge_report_rejects_an_identical_stamp_claiming_two_different_values() {
        ...
        // so this must be rejected outright, not silently resolved
        // either way.
        let mut a: LwwMap<String, String> = LwwMap::new();
        a.set("node-1".to_string(), "ok".to_string(), LamportTime(3), 1);
        let mut b: LwwMap<String, String> = LwwMap::new();
        ...
        let err = a.merge_report(&b).unwrap_err();
    ...
    #[test]
    fn merge_report_still_treats_a_true_identical_duplicate_as_idempotent() {
        // The counterpart to the rejection test above: same stamp AND
        // same value (e.g. a retried gossip round) is the legitimate
        // idempotent case and must NOT be rejected.
        let mut a: LwwMap<String, String> = LwwMap::new();
        a.set("node-1".to_string(), "ok".to_string(), LamportTime(3), 1);
        let mut b: LwwMap<String, String> = LwwMap::new();
        b.set("node-1".to_string(), "ok".to_string(), LamportTime(3), 1);
